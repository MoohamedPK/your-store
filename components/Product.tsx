...
const Product = ({product}: {product: Product}) => {
  ...
  return (
    <div className="px-4 py-6 border-2 border-black cursor-pointer group shadow-2xl shadow-neutral-500/80">
        <Link href={`/products/${id}`} key={id} className="space-y-3">
            ...
            <Image src={image} alt="prod image" width={350} height={350} className="group-hover:scale-85 transition-transform duration-300"/>
        </Link>
        <AddToCartBtn/>
    </div>
  )
